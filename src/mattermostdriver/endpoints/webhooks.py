...
class Webhooks(Base):
    ...
    def update_incoming_hook(self, hook_id, options):
        return self.client.put(self.endpoint + "/incoming/" + hook_id, options=options)

    def create_outgoing_hook(self, options):
        return self.client.post(self.endpoint + "/outgoing", options=options)

    def list_outgoing_hooks(self, params):
        return self.client.get(self.endpoint + "/outgoing", params=params)

    def get_outgoing_hook(self, hook_id):
        return self.client.get(self.endpoint + "/outgoing/" + hook_id)

    def delete_outgoing_hook(self, hook_id):
        return self.client.delete(self.endpoint + "/outgoing/" + hook_id)

    def update_outgoing_hook(self, hook_id, options):
        return self.client.put(self.endpoint + "/outgoing/" + hook_id, options=options)
